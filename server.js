...
async function scrapeBeforward(page) {
  return await page.evaluate(() => {
    ...
    if (!year) { const m = (document.title.match(/\b(19|20)\d{2}\b/) || [])[0]; if (m) year = parseInt(m); }
    const bodyText = document.body.innerText;
    let cc = null;
    const ccPatterns = [/(\d{1,2}[.,\s]\d{3}|\d{3,4})\s*cc\b/i, /(\d{1,2}[.,\s]\d{3}|\d{3,4})cc/i, /(\d[.,]\d)\s*[lL]\b/, /(?:displacement|engine|cilindrada|motor|engine size)[^\d]{0,20}(\d{1,2}[.,\s]\d{3}|\d{3,4})/i, /(?:^|\s)(\d{1,2}[.,]\d{3})(?:\s|$)/, /\b(\d{3,4})\b/];
